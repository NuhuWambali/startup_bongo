        ...
        <div class="card card-primary">
            ...
            <div class="card-body">
                <form action="{{ route('admin.ad.update', 1) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <h5 class="text-primary">{{ __('admin.Articles View Page Ads') }}</h5>
                    ...
                    <h5 class="text-primary">{{ __('admin.Articles Page Ads') }}</h5>

                    <div class="form-group">
                        <img src="{{ asset($ad->articles_page_ad) }}" width="200px" alt="">
                        <br>
                        <label for="">{{ __('admin.Bottom Ad') }}</label>
                        <input name="articles_page_ad" type="file" class="form-control" >
                        @error('articles_page_ad')
                            <p class="text-danger">{{ $message }}</p>
                        @enderror

                        <label for="" class="mt-3">{{ __('admin.Bottom Ad Url') }}</label>
                        <input name="articles_page_ad_url"  value="{{ $ad->articles_page_ad_url }}" type="text" class="form-control" >
                        @error('articles_page_ad_url')
                            <p class="text-danger">{{ $message }}</p>
                        @enderror
                        ...
                        <label class="custom-switch mt-2">
                            <input
                            {{ $ad->articles_page_ad_status == 1 ? 'checked' : '' }}
                                name="articles_page_ad_status"
                                value="1" type="checkbox" class="custom-switch-input toggle-status">
                            <span class="custom-switch-indicator"></span>
                        </label>
                    </div>
