        ...
        <div class="card card-primary">
            ...
            <div class="card-body">
                ...
                <div class="tab-content tab-bordered" id="myTab3Content">
                    @foreach ($languages as $language)
                        @php
                            if(canAccess(['articles all-access'])){
                                                    ...
                                                    @if (canAccess(['articles status', 'articles all-access']))
                                                        <td>
                                                            <label class="custom-switch mt-2">
                                                                <input {{ $item->is_breaking_articles === 1 ? 'checked' : '' }}
                                                                    data-id="{{ $item->id }}" data-name="is_breaking_articles"
    ...
    <script>
        ...
        $(document).ready(function(){
            $('.toggle-status').on('click', function(){
                ...
                $.ajax({
                    method: 'GET',
                    url: "{{ route('admin.toggle-articles-status') }}",
                    data: {
                        id:id,
                        name:name,
                        status:status
